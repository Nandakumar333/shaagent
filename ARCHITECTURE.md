...
1. **Single Template, All Platforms** — one generic template set generates agent configs for
   OpenCode, GitHub Copilot, Claude Code, Codex, Cursor, and Continue. Platform-specific
   formatting (frontmatter, file structure, extensions) is handled by the template engine.
...
4. **Incremental adoption** — start with the 7 core sub-agents; opt-in to advanced ones
   (Security, ArchReview, MemoryCreator) per repo.
...
│   │   │   ├── platforms/
│   │   │   │   └── index.ts        # unified platform path resolver (all 6 platforms)
│   │   │   └── engine/
...
| Platform | Agent files location | Root instruction file | Config | Extension |
|----------|---------------------|----------------------|--------|-----------|
| OpenCode | `.opencode/agents/` | — | `opencode.json` | `.md` |
| Claude Code | `.claude/agents/` | `CLAUDE.md` (with `@` imports) | `CLAUDE.md` | `.md` |
| GitHub Copilot | `.github/instructions/` | `AGENTS.md` (repo root) | `.github/copilot-instructions.md` | `.instructions.md` |
| Codex (OpenAI) | repo root (merged) | `AGENTS.md` (all-in-one) | — | — |
| Cursor | `.cursor/rules/` | — | `.cursorrules` | `.mdc` |
| Continue | `.continue/prompts/` | — | `.continue/config.json` | `.md` |
...
| Feature | OpenCode | Claude Code | GitHub Copilot | Codex | Cursor | Continue |
|---------|----------|-------------|----------------|-------|--------|----------|
| Individual agent files | ✅ | ✅ | ✅ | ❌ (merged) | ✅ | ✅ |
| YAML frontmatter | ✅ | ❌ | ✅ (`applyTo`) | ❌ | ✅ | ❌ |
| Root instruction file | ❌ | ✅ `CLAUDE.md` | ✅ `AGENTS.md` | ✅ `AGENTS.md` | ❌ | ❌ |
