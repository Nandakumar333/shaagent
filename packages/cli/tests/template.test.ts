...
describe('engine/template — renderAgents', () => {
  ...
  describe('github-copilot platform', () => {
    it('should render with .instructions.md extension', async () => {
      const answers = makeAnswers('github-copilot');
      const written = await renderAgents(answers);

      // Agent files should have .instructions.md extension
      const agentFiles = written.filter(f => f.includes('instructions'));
      expect(agentFiles.length).toBe(7);
      agentFiles.forEach(f => {
        expect(f).toMatch(/\.instructions\.md$/);
      });
    });
    ...
    it('should include applyTo frontmatter', async () => {
      ...
      const agentPath = path.join(tempDir, '.github', 'instructions', 'orchestrator.instructions.md');
