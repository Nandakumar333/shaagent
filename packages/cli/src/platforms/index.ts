...
export function getPlatformPaths(platform: Platform, scope: Scope = 'project'): PlatformPaths {
  ...
  const map: Record<Platform, PlatformPaths> = {
    ...
    'github-copilot': scope === 'global' ? {
      agentsDir:  path.join(base, '.config', 'github-copilot', 'agents'),
      skillsDir:  path.join(base, '.config', 'github-copilot', 'skills'),
    ...
    'github-copilot-cli': {
      ...
      skillsDir:  scope === 'global' ? path.join(base, '.copilot', 'skills') : path.join(base, '.github', 'skills'),
      configFile: 'copilot-instructions.md',
      extension:  '.agent.md',
      mergedFile: false,
    },
