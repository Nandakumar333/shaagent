...
import type { RenderOptions } from '../init';
...
/**
 * GitHub Copilot format: YAML frontmatter with applyTo for .instructions.md files
 */
function buildCopilotFormat(
  ...
  // Copilot .instructions.md files use applyTo for scope
  const frontmatter = [
    '---',
    `applyTo: "**"`,
    '---',
...
function generateRootInstruction(
  ...
  if (platform === 'claude-code') {
    ...
    lines.push(`Agent instructions are in \`.github/instructions/\`.`);
    lines.push('');
    if (context.techStack) lines.push(`**Tech Stack:** ${context.techStack}`);
    if (context.infrastructure) lines.push(`**Infrastructure:** ${context.infrastructure}`);
    lines.push('');
